...
export function ExecutionPanel({ runId, timeline, events, onAnswer, onCancel, onRetry }: { runId: string | null; timeline: Timeline | null; events: EventOut[]; onAnswer: (answer: string) => Promise<void>; onCancel: () => Promise<void>; onRetry: () => Promise<void> }) {
  const [answer, setAnswer] = useState("");
  const [trace, setTrace] = useState(false);
  if (!runId || !timeline) {
    return (
      <aside className="w-80 shrink-0 border-l border-border bg-surface p-3 text-xs text-muted">
